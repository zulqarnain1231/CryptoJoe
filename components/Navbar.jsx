...
const Navbar = ({ click, isdark }) => {
  //  const handleClick =()=>{
  //    click ;
  //  }
  return (<>
    <div className={`hidden lg:flex justify-between z-10 fixed w-full ${isdark ? 'bg-[#181823]' : 'bg-[#F3EEEA] '}  p-2'`}>
      <Link href='/' >
      ...
      <div>
        {
          isdark ?
            <button onClick={click} className='p-2 mt-1 hover:bg-slate-600 text-white rounded-full bg-slate-700 '><BsFillSunFill size={20} /></button>
            :
            <button onClick={click} className='p-2 mt-1 hover:bg-[#ECECFE] rounded-full bg-white '><BiMoon size={20} /></button>
        }
      </div>

      <div>
        <button className={`flex gap-1 mt-1 rounded-full ${isdark ? 'bg-slate-700 hover:bg-slate-600 text-white': 'bg-white hover:bg-[#ECECFE]'} py-2  px-4`}>
    ...
    <div className={`flex lg:hidden justify-between z-10 fixed w-full ${isdark ? 'bg-[#181823]' : 'bg-[#F3EEEA]'}  p-2`}>
      <Link href='/' >
        <div className='h-10 w-10 py-0'>
          <Image className='cursor-pointer hover:-rotate-6 duration-150 object-cover h-full w-full mt-2' src={logo} />
